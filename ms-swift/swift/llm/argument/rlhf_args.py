...
class RLHFArguments(TeacherModelArguments, GRPOArguments, PPOArguments, RewardModelArguments, RLHFArgumentsMixin,
    ...
    Args:
        ...
        max_new_tokens (Optional[int]): A backward-compatibility argument. Please use `max_completion_length` instead.
            Defaults to None.
    """
    rlhf_type: Literal['dpo', 'orpo', 'simpo', 'kto', 'cpo', 'rm', 'ppo', 'grpo', 'gkd', 'dgo'] = 'dpo'
    ...
    # DGO (Distribution-Guided Optimization)
    dgo_beta: float = 0.1  # Temperature for DGO weighting (lower = sharper distribution)
    dgo_data_file: Optional[str] = None  # Path to pre-generated DGO data file
    dgo_output_file: Optional[str] = None  # Path to save generated data (generation phase)
    dgo_recompute_weights: bool = False  # Whether to recompute weights from rewards
    freeze_router: bool = False  # Freeze router/gate parameters (Group D experiments)
    reward_style: Literal['swift', 'gfol'] = 'swift'  # Reward function style
    ...
    def _init_dgo(self):
        """Initialize DGO-specific parameters"""
        if self.rlhf_type != 'dgo':
            return
        ...
        # DGO is offline, doesn't need online generation
        self.remove_unused_columns = False

        # Log DGO configuration
        logger.info(f'DGO Configuration:')
        logger.info(f'  dgo_beta: {self.dgo_beta}')
        logger.info(f'  dgo_data_file: {self.dgo_data_file}')
        logger.info(f'  freeze_router: {self.freeze_router}')
        logger.info(f'  reward_funcs: {self.reward_funcs}')
