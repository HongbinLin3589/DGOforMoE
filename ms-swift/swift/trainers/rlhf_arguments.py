...
@dataclass
class DGOConfig(RolloutTrainerArgumentsMixin, SwiftArgumentsMixin, HfTrainingArguments):
    """
    Configuration for DGO (Distribution-Guided Optimization) training.
    ...
    Args:
        num_generations: Number of responses to generate per prompt (default: 8)
        temperature: Sampling temperature for generation (default: 0.8)
        dgo_output_file: Path to save generated data (generation phase)
        dgo_beta: Temperature for DGO weighting (lower = sharper, default: 0.1)
        dgo_data_file: Path to pre-generated data file (training phase)
        dgo_recompute_weights: Whether to recompute weights from rewards (default: False)
        freeze_router: Freeze router/gate parameters for Group D experiments (default: False)
        reward_funcs: List of reward function names (e.g., ['accuracy'])
        reward_style: Reward function style ('swift' for 1.0 max, 'gfol' for 1.5 max)
        max_completion_length: Maximum length for generated completions (default: 1024)
    """
    # DGO Generation
    num_generations: int = 8
    temperature: float = 0.8
    dgo_output_file: Optional[str] = None
    max_completion_length: int = 1024

    # DGO Training
    dgo_beta: float = 0.1
    dgo_data_file: Optional[str] = None
    dgo_recompute_weights: bool = False
    freeze_router: bool = False  # For Group D experiments (frozen router)

    # Reward
    reward_funcs: Optional[List[str]] = None
    reward_style: Literal['swift', 'gfol'] = 'swift'
    ...
    def __post_init__(self):
        ...
        # Validate beta
        if self.dgo_beta <= 0:
            raise ValueError(f'dgo_beta must be positive, got {self.dgo_beta}')
